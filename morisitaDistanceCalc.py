...
def Morisita_Dist():
    cwd = input("Please enter the location on your computer that your file you would like to compute Morisita Distance with can be found. For instance, if the file is stored on your desktop, please enter the filepath up to the word desktop, (if on a windows computer, remember to remove quotation marks!): ")
    os.chdir(cwd)
    ...
    Timepoint1 = pd.read_csv(Patient, index_col=(0), header=(0))
    print(Timepoint1)
    Timepoint1.dropna()
    T1 = int(T1)
    T2 = int(T2)
    ...
    Timepoint2 = pd.read_csv(Patient, index_col=(0), header=(0))
    Timepoint2.dropna()
